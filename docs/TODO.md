...
- [ ] **Audit test coverage and write unit tests where missing.** No test runner/tests exist anywhere in the repo (see Known gaps below) — needs an inventory of what most needs coverage (parsers — `gpx/parser.js`, `igc/parser.js`, `slpz/parser.js` — are the highest-value target, being pure functions with tricky edge cases) before picking a test runner and writing tests. Docs (`CLAUDE.md`/`docs/*.md`) should gain a "running tests" section once a runner exists, and new-feature docs going forward should note what tests back them.

## Done

- [x] **`.slpz`/`.slopes` file support (Slopes ski app exports)** (2026-08-06) — new `slpz/parser.js` unzips the export via `adm-zip` and parses its headerless `GPS.csv` (format reverse-engineered from [wfraser/slopes-gpx](https://github.com/wfraser/slopes-gpx), no official spec, no sample file available to verify against); reuses `igc/parser.js`'s `haversineMeters` for distance/speed/elevation stats. `processor.js`/watcher dispatch by extension. Title/type editing stays `.gpx`-only; activity type fixed to "Skiing".
