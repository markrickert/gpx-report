...
- [ ] **Dark-mode map background flashes white before tiles load.** `ActivityDetail.jsx`'s Leaflet map (and likely the Heatmap page's) has no dark-aware background set on the map container itself, so the default white `.leaflet-container` background shows through until CARTO's dark tiles finish loading, flashing bright in dark mode. Likely fixed with a dark background-color on `.activity-map`/`.leaflet-container` under `:root[data-theme="dark"]`.
