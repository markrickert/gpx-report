...
*   **The dashboard's "Code" tab (`frontend/src/pages/CodeEditor.jsx`) only works over plain http.** It iframes `http://<hostname>:8443` (code-server has no TLS). If you're viewing the dashboard through the public https Caddy domain (§6), the browser blocks/refuses that iframe as mixed content — this is inherent to code-server having no TLS and can't have a public Caddy site (see above), not a bug to fix per-request. Reach the dashboard over plain http (Tailscale IP or LAN) for the Code tab to load.
