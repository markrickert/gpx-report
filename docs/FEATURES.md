...
*   **Route Data Extraction:** Stores sequences of latitude, longitude, elevation, and timestamp for each activity. For GPX files, also extracts per-point heart rate, cadence, and ambient temperature from Garmin's `<gpxtpx:TrackPointExtension>` when present (most activities have none — GPS-only, no paired sensor); average/max heart rate are computed once at ingest and stored on `activities`. IGC/`.skiz` don't carry this extension at all.
...
*   **Activity Type Editing:** Same inline Edit/Save/Cancel affordance next to the activity-type badge, backed by a dropdown of the same preselected activity types used for the dashboard filter; saving rewrites the `<trk><type>` element (`.gpx`) or `Track.xml`'s `activity` attribute (`.skiz`) and re-runs processing. Same `.gpx`/`.skiz` availability as title editing. When the activity's current type is "Unknown", up to 3 heuristic-ranked suggestion chips appear above the dropdown (backend's `Activity.suggestedActivityTypes`, `backend/src/track/suggestType.ts` — ranks candidate types by how well the track's own avg/max speed and elevation-gain/loss-per-km fit rough per-type bands, not the flat alphabetical list) so the likeliest types are one tap away instead of scrolling the full dropdown.
...
*   **Heart Rate Chart & Stats:** When the source GPX carried heart-rate data (`<gpxtpx:hr>`), two additional metric tiles (Avg/Max Heart Rate) and a "Heart Rate" line chart appear below the Elevation Profile, synced to the same hover position (see below). Hidden entirely for the large majority of activities that have no HR data, rather than showing an empty chart.
