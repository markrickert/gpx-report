...
*   **Map View:** An interactive map displaying the geographical path of the activity, colored by speed — the route is drawn as up to 150 chunked segments (grouped for pan/zoom performance on dense tracks), each colored along a red(slow)→blue(fast) scale by that stretch's average speed, with a min/max speed legend below the map. Same color scale as the elevation chart's speed gradient (below) for a consistent read between the two. The map also supports rotating away from north — a two-finger touch rotate gesture, or shift+scroll wheel on desktop (via `leaflet-rotate`) — and the existing reset-view button resets bearing back to north alongside its pan/zoom reset.
