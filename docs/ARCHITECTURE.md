...
*   **Purpose:** Handles the parsing of raw GPX/IGC/Slopes files and populates the PostgreSQL database.
*   **Trigger:** A `chokidar` file watcher (`backend/src/gpx/watcher.js`) watches `GPX_FILES_DIRECTORY` for `.gpx`, `.igc`, `.slpz`, and `.slopes` files; on startup it fires an `add` event for every pre-existing file, then continues watching for new ones. Files land in that directory either externally (Syncthing/manual drop) or via the `saveRecordedActivity` mutation (Record page) writing a new file into the same directory — either way, the watcher is the single trigger into this pipeline.
*   **Components:**
    ...
    *   **Slopes Parser** (`backend/src/slpz/parser.js`): For `.slpz`/`.slopes` exports from the Slopes ski-tracking app. The format is a zip archive containing a headerless `GPS.csv` (timestamp, lat, lon, elevation, course, speed, horizontal/vertical accuracy per line) plus a `Metadata.xml` this parser doesn't read; unzipped via `adm-zip`. Reuses `igc/parser.js`'s `haversineMeters` for distance/speed/elevation stats. No track-name equivalent, so title falls back to the filename stem and activity type is fixed to "Skiing". Column layout is reverse-engineered (no official spec) per [wfraser/slopes-gpx](https://github.com/wfraser/slopes-gpx) — flag if real Slopes exports parse incorrectly.
...
1.  User adds GPX, IGC, or Slopes (`.slpz`/`.slopes`) files to a monitored directory (manually, or synced from a phone via Syncthing — see `docs/SETUP.md` §6).
